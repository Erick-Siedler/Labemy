...
<div class="container-info group-dashboard">
    <div class="owner-shell">
        <div class="owner-tabs" role="tablist" aria-label="Seções do grupo">
            <button type="button" class="owner-tab is-active" data-owner-tab="dashboard" role="tab" aria-selected="true" aria-controls="group-panel-dashboard">dashboard</button>
            <button type="button" class="owner-tab" data-owner-tab="members" role="tab" aria-selected="false" aria-controls="group-panel-members">membros</button>
            <button type="button" class="owner-tab" data-owner-tab="projects" role="tab" aria-selected="false" aria-controls="group-panel-projects">projetos</button>
        </div>

        <div class="owner-panel">
            <section class="owner-panel-section" id="group-panel-dashboard" data-owner-panel="dashboard" role="tabpanel">
                ...
                <div class="graph-grid" data-animate>
                    <div class="graph-card">
                        ...
                        <div class="graph-body">
                            <div class="chart-wrap chart-bar">
                                <canvas id="groupProjectBar" class="chart-canvas" data-series='@json($projectStatusChart)'></canvas>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="graph-card">
                        ...
                        <div class="graph-body">
                            <div class="chart-wrap chart-bar">
                                <canvas id="groupVersionBar" class="chart-canvas" data-series='@json($versionStatusChart)'></canvas>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <section class="owner-panel-section" id="group-panel-members" data-owner-panel="members" role="tabpanel" hidden>
                <div class="board-grid" data-animate>
                    <div class="board-card">
                        <div class="board-header">
                            <h3>Membros do grupo</h3>
                        ...
                        <div class="board-body">
                            @forelse ($groupMembers as $member)
                                <div class="member-item">
                                    ...
                                    @php
                                        $memberRole = $member->role === 'asssitant' ? 'assistant' : $member->role;
                                    @endphp
                                    @if($canManageMembers)
...
@else
    <span class="status-badge status-{{ $memberRole }}">{{ ucfirst($memberRole) }}</span>
@endif
                                </div>
                            @empty
            ...
            <section class="owner-panel-section" id="group-panel-projects" data-owner-panel="projects" role="tabpanel" hidden>
                <div class="projects-section" data-animate>
                    <div class="section-header">
                        <h3>Projetos do grupo</h3>
                        <span class="section-count">{{ $groupProjects->count() }}</span>
                    </div>
                    ...
                    <div class="project-grid">
                        @forelse ($groupProjects as $project)
                            @php
                                ...
                                $latestStatusValue = $latest?->status_version ?? 'draft';
                                $versionStatus = 'status-' . str_replace('_', '-', $latestStatusValue);
                            @endphp
                            <article class="project-card">
                                <div class="project-card-header">
                                    <div>
                                        <h4>{{ $project->title }}</h4>
                                        <span class="project-sub">ID {{ $project->id }}</span>
                                    </div>
                                ...
                                <div class="project-meta">
                                    <div class="meta-line">
                                        ...
                                        <strong>{{ $latest ? $latest->version_number : 'Sem Versão' }}</strong>
                                    </div>
                                    <div class="meta-line">
                                        <span>Status da Versão</span>
                                        <span class="status-badge {{ $versionStatus }}">{{ $versionStatusLabels[$latestStatusValue] ?? $latestStatusValue }}</span>
                                    </div>
                                    <div class="meta-line">
                                        <span>Submetida</span>
                                        <strong>{{ $latest->submitted_at ?? '-' }}</strong>
                                    </div>
                                    <div class="meta-line">
                                        <span>Aprovada</span>
                                        <strong>{{ $latest->approved_at ?? '-' }}</strong>
                                    </div>
                                </div>
                                <a class="btn-view" href="{{ route('project.index', $project->id) }}">Ver projeto</a>
                            </article>
                        @empty
                            <div class="table-empty">Nenhum projeto encontrado.</div>
                        @endforelse
                    </div>
                </div>
            </section>
