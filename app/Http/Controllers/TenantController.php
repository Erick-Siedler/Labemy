...
class TenantController extends Controller
{
     ...
     */
    public function index(TenantRelationService $relationService)
    {
        if (!Auth::check()) {
        ...
        $hasOwnerTenantAccess = (bool) $tenant || UserRelation::where('user_id', $user->id)
            ->where('status', 'active')
            ->where('role', 'owner')
            ->exists();

        $hasAccessibleTenants = (bool) $tenant || UserRelation::where('user_id', $user->id)
            ->where('status', 'active')
            ->exists();
        ...
        return view('main.index-plans', [
            ...
            'hasAccessibleTenants' => $hasAccessibleTenants,
            'hasOwnerTenantAccess' => $hasOwnerTenantAccess,
        ]);
    }

    /**
     * Carrega os dados necessarios para a criacao de registros.
     */
     ...
     */
    public function store(Request $request, string $token, TenantRelationService $relationService)
    {
        ...
        $relationService->ensureOwnerRelation($user, $tenant);
        $relationService->activateTenant($user, (int) $tenant->id);
