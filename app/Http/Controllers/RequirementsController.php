...
class RequirementsController extends Controller
{
    ...
    {
        return app(RequirementsService::class)->index($request, $project, $homeOwnerData, $subHomeData);
    ...
    /**
     * Executa a rotina 'export' no fluxo de negocio.
     */
    public function export(Project $project)
    {
        return app(RequirementsService::class)->export($project);
    ...
    /**
     * Executa a rotina 'storeFunc' no fluxo de negocio.
     */
    public function storeFunc(StoreFuncReqRequest $request, Project $project): RedirectResponse
    {
        return app(RequirementsService::class)->storeFunc($request, $project);
    ...
    /**
     * Executa a rotina 'updateFunc' no fluxo de negocio.
     */
    public function updateFunc(UpdateFuncReqRequest $request, Project $project, FuncReq $funcReq): RedirectResponse
    {
        return app(RequirementsService::class)->updateFunc($request, $project, $funcReq);
    ...
    /**
     * Executa a rotina 'destroyFunc' no fluxo de negocio.
     */
    public function destroyFunc(Request $request, Project $project, FuncReq $funcReq): RedirectResponse
    {
        return app(RequirementsService::class)->destroyFunc($request, $project, $funcReq);
    ...
    /**
     * Executa a rotina 'storeNonFunc' no fluxo de negocio.
     */
    public function storeNonFunc(StoreNonFuncReqRequest $request, Project $project): RedirectResponse
    {
        return app(RequirementsService::class)->storeNonFunc($request, $project);
        ...
        $this->logAction(
            ...
            'non_func_req',
            (int) $nonFuncReq->id,
            'Requisito não funcional criado: ' . $nonFuncReq->code . ' - ' . $nonFuncReq->title . '.'
        );

        return $this->redirectToRequirementsIndex($projectData, $request)
            ->with('success', 'Requisito não funcional criado.');
    }

    /**
     * Executa a rotina 'updateNonFunc' no fluxo de negocio.
     */
    public function updateNonFunc(UpdateNonFuncReqRequest $request, Project $project, NonFuncReq $nonFuncReq): RedirectResponse
    {
        return app(RequirementsService::class)->updateNonFunc($request, $project, $nonFuncReq);
        ...
        $this->logAction(
            ...
            'non_func_req',
            (int) $nonFuncReq->id,
            'Requisito não funcional atualizado: ' . $nonFuncReq->code . ' - ' . $nonFuncReq->title . '.'
        );

        return $this->redirectToRequirementsIndex($projectData, $request)
            ->with('success', 'Requisito não funcional atualizado.');
    }

    /**
     * Executa a rotina 'destroyNonFunc' no fluxo de negocio.
     */
    public function destroyNonFunc(Request $request, Project $project, NonFuncReq $nonFuncReq): RedirectResponse
    {
        return app(RequirementsService::class)->destroyNonFunc($request, $project, $nonFuncReq);
        ...
        $this->logAction(
            ...
            'non_func_req',
            (int) $nonFuncReq->id,
            'Requisito não funcional removido: ' . $code . ' - ' . $title . '.'
        );

        return $this->redirectToRequirementsIndex($projectData, $request)
            ->with('success', 'Requisito não funcional removido.');
    }
    ...
    private function buildLayoutData(array $actor, Project $project, HomeOwnerDataService $homeOwnerData, SubHomeDataService $subHomeData): array
    {
        ...
        $data['theme'] = $this->resolveTheme($data['userPreferences'] ?? null);
    ...
    /**
     * Executa a rotina 'resolveProjectBackUrl' no fluxo de negocio.
     */
    ...
    /**
     * Executa a rotina 'loadAuthorizedProject' no fluxo de negocio.
     */
    ...
    /**
     * Executa a rotina 'ensureSubUserCanAccessProject' no fluxo de negocio.
     */
    ...
    /**
     * Executa a rotina 'ensureFuncReqBelongsToProject' no fluxo de negocio.
     */
    ...
    /**
     * Executa a rotina 'ensureNonFuncReqBelongsToProject' no fluxo de negocio.
     */
    ...
    /**
     * Executa a rotina 'normalizeIds' no fluxo de negocio.
     */
    ...
    /**
     * Executa a rotina 'assertFuncReqIdsBelongToProject' no fluxo de negocio.
     */
    ...
    /**
     * Executa a rotina 'requirementsPaginationStateFromRequest' no fluxo de negocio.
     */
    ...
    /**
     * Executa a rotina 'redirectToRequirementsIndex' no fluxo de negocio.
     */
    ...
    /**
     * Executa a rotina 'resolveFuncCode' no fluxo de negocio.
     */
    private function resolveFuncCode(string $incomingCode, int $tenantId, int $projectId, ?int $ignoreId = null): string
    {
        ...
        if ($query->exists()) {
            throw ValidationException::withMessages([
                'code' => 'Código RF já utilizado neste projeto.',
            ]);
        }
    ...
    /**
     * Executa a rotina 'resolveNonFuncCode' no fluxo de negocio.
     */
    private function resolveNonFuncCode(string $incomingCode, int $tenantId, int $projectId, ?int $ignoreId = null): string
    {
        ...
        if ($query->exists()) {
            throw ValidationException::withMessages([
                'code' => 'Código RNF já utilizado neste projeto.',
            ]);
        }
    ...
    /**
     * Executa a rotina 'nextCode' no fluxo de negocio.
     */
    ...
    /**
     * Executa a rotina 'logAction' no fluxo de negocio.
     */
    ...
    /**
     * Define se o tenant informado eh solo e pertence ao usuario autenticado.
     */
    ...
    /**
     * Executa a rotina 'readOnlyRedirect' no fluxo de negocio.
     */
