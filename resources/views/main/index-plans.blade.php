...
@php
    ...
    $hasTenantAccess = (bool) ($hasAccessibleTenants ?? false);
    $hasOwnerAccess = (bool) ($hasOwnerTenantAccess ?? false);
    $fromSettings = request()->query('from') === 'settings';
    $profileReturnUrl = ((Auth::user()->role ?? 'owner') === 'owner')
        ? route('profile')
        : route('subuser-settings') . '#perfil';
@endphp
@if($fromSettings)
<div class="plan-banner">
    <strong>Mudou de ideia?</strong>
    <span>Se nao quiser trocar de plano agora, volte para seu perfil.</span>
    <div style="margin-top: 8px;">
        <a class="bt" href="{{ $profileReturnUrl }}">Voltar para o perfil</a>
    </div>
</div>
@endif
<div class="plan-banner">
...
@if($hasTenantAccess && !$hasOwnerAccess)
<div class="plan-banner">
    <strong>Voce ja participa de tenants</strong>
    <span>Se quiser, voce pode acessar um tenant existente sem contratar plano.</span>
    <div style="margin-top: 8px;">
        <a class="bt" href="{{ route('tenant.select.index') }}">Acessar meus tenants</a>
    </div>
</div>
@endif
@endauth
    ...
    @for ($i = 1;$i < 4; $i++)
        ...
        <div class="card-top">
            @if ($i === 1)
                <h1>Plano Solo</h1>
            @elseif ($i === 2)
                <span class="plan-badge plan-badge--popular">Mais escolhido</span>
                <h1>Plano Pro</h1>
            @else
